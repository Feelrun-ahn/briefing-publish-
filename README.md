...
PC에서 이어 작업할 때 파일이 나뉘어 있는 쪽이 편해서 함께 넣어 두었습니다.
...
카드를 고치려면 `app.js`에서 `/* ══════ js/cards/이름.js ══════ */` 주석을 찾으세요.
...
- **알림** — 시각 있는 항목은 N분 전, 없는 항목은 아침에 모아서, 내일 일정은 전날 밤에
- **기기 연결** — QR 또는 6자리 코드. 일정·할 일·루틴·시간표·카드 구성이 공유됨
- **운세** — 절기·일진·십신을 계산하는 사주 (재미로 보는 참고용, 외부 통신 없음)
...
- [ ] 앱 이름 확정 → 패키지 ID 결정
- [ ] Capacitor 패키징 (알림을 앱 종료 상태에서도 받으려면 필요)
- [ ] 개인정보처리방침 페이지
- [ ] 폐쇄 테스트 12명 × 14일
